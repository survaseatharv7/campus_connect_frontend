...
export default function HODClubRequestsPage() {
  ...
  const approveMutation = useMutation({
    ...
    onSuccess: () => { toast.success('Club approved! Sent to Principal.'); queryClient.invalidateQueries({ queryKey: ['hod-club-requests'] }) },
    onError: (err) => toast.error(err.response?.data?.message || 'Failed'),
  })

  const rejectMutation = useMutation({
    mutationFn: (id) => hodAPI.rejectClub(id),
    onSuccess: () => { toast.success('Club rejected'); queryClient.invalidateQueries({ queryKey: ['hod-club-requests'] }) },
    onError: (err) => toast.error(err.response?.data?.message || 'Failed'),
  })

  const tabs = [
    { key: 'PENDING_HOD', label: 'Pending', icon: Clock },
    { key: 'PENDING_PRINCIPAL', label: 'Sent to Principal', icon: CheckCircle },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 border-b border-dark-100">
        {tabs.map((t) => (
          <button key={t.key} onClick={() => setTab(t.key)} className={`flex items-center gap-2 px-4 py-2.5 text-sm font-medium rounded-t-xl whitespace-nowrap transition-colors ${tab === t.key ? 'bg-primary-50 text-primary-600 border-b-2 border-primary-600' : 'text-dark-500 hover:text-dark-700'}`}>
      ...
      {isLoading ? (
        ...
        <EmptyState icon={Award} title="No clubs here" description={`No club requests with status "${formatEnumLabel(tab)}".`} />
      ) : (
            ...
            <motion.div key={club.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.05 }}>
              <Card>
                ...
                <div className="flex items-center gap-2 text-xs text-dark-400 mb-4">
                  ...
                  <span>→</span>
                  <span className={`px-2 py-0.5 rounded ${club.status === 'PENDING_HOD' ? 'bg-amber-50 text-amber-700' : 'bg-green-50 text-green-700'}`}>
                    {club.status === 'PENDING_HOD' ? '⏳' : '✓'} HOD
                  </span>
                  <span>→</span>
                  <span className="px-2 py-0.5 rounded bg-gray-100 text-gray-500">Principal</span>
                </div>
