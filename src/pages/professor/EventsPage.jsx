  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div><h1 className="text-2xl font-bold font-heading text-dark-900">My Events</h1><p className="text-dark-500 text-sm mt-1">Create and manage events</p></div>
      ...
      <Modal isOpen={createOpen} onClose={() => { setCreateOpen(false); form.reset() }} title="Create Event" size="lg" footer={<><Button variant="secondary" onClick={() => setCreateOpen(false)}>Cancel</Button><Button onClick={form.handleSubmit((d) => createMutation.mutate(d))} loading={createMutation.isPending}>Create</Button></>}>
        <form className="space-y-4">
          <Input label="Title" placeholder="e.g. Workshop on ML" error={form.formState.errors.title?.message} {...form.register('title')} />
          ...
          <Input label="Venue" placeholder="Lab/Room" error={form.formState.errors.venue?.message} {...form.register('venue')} />
